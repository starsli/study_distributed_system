...
func (p *Participant3PC) handleMessage(msg Message3PC) {
	...
	case CanCommit:
		...
		if p.canCommit {
			p.state = Waiting
			fmt.Printf("[Participant %d] 状态变更: Idle -> Waiting\n", p.ID)
			p.sendResponse(msg.TransactionID, Yes)
		} else {
			p.state = Aborted
			fmt.Printf("[Participant %d] 状态变更: Idle -> Aborted\n", p.ID)
	...
	case PreCommit:
		if p.state == Waiting {
			...
			p.state = Prepared
			p.persistent = true
			fmt.Printf("[Participant %d] 状态变更: Waiting -> Prepared (持久化成功)\n", p.ID)
	...
	case Abort3PC:
		if p.state == Waiting || p.state == Prepared {
			p.state = Aborted
			fmt.Printf("[Participant %d] 状态变更: %s -> Aborted\n", p.ID, p.state)
		}
	}
}

func (p *Participant3PC) sendResponse(txnID string, response MsgType3PC) {
	fmt.Printf("[Participant %d] 发送响应: %s\n", p.ID, response)
...
func (c *Coordinator3PC) handleMessage(msg Message3PC) {
	...
	case Ready:
		c.readyCount++
		fmt.Printf("[Coordinator] 累计Ready: %d/%d\n", c.readyCount, len(c.participants))
	}
...
func (c *Coordinator3PC) phase1CanCommit(txnID string) {
	...
	fmt.Println("\n========== 第一阶段: CanCommit 阶段 ==========")
	fmt.Printf("[Coordinator] 发起事务 %s，向所有参与者发送 CanCommit\n", txnID)
...
func (c *Coordinator3PC) phase2PreCommit() {
	...
	fmt.Println("\n========== 第二阶段: PreCommit 阶段 ==========")
	fmt.Println("[Coordinator] 所有参与者都同意，发送 PreCommit 命令")
...
func (c *Coordinator3PC) phase3DoCommit() {
	...
	fmt.Println("\n========== 第三阶段: DoCommit 阶段 ==========")
	fmt.Println("[Coordinator] 所有参与者都Ready，发送 DoCommit 命令")
